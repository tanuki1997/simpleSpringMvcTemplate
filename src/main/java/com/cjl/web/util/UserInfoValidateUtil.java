...
public class UserInfoValidateUtil {
     ...
     * 身份证有效
     */
    public static final String VALIDITY = "SUCCESS";
    /**
     * 位数不足
     */
     ...
     *
     *  个人理解: 通过前17位加权(* 权值wi[位数]) 总和 取余11得到值Y, 再从varifyCode[Y]获取校验码得到第18位 如果与最后一位相同即正确
     */
    private static boolean isVarifyCode(String Ai, String IDStr) {
        ...
        int sum = 0;
        for (int i = 0; i < 17; i++) {
            System.out.println(String.format("%d = %d * %d", sum, Integer.parseInt(String.valueOf(Ai.charAt(i))), Integer.parseInt(Wi[i])));
            sum = sum + Integer.parseInt(String.valueOf(Ai.charAt(i))) * Integer.parseInt(Wi[i]);
        }
        int modValue = sum % 11;
        System.out.println("Y值:" + modValue);

        String strVerifyCode = VarifyCode[modValue];
        System.out.println("校验码:" + strVerifyCode);
        Ai = Ai + strVerifyCode;
        if (IDStr.length() == 18) {
    ...
    public static void main(String[] args) {
        System.out.println(IDCardValidate("431028199701121011"));
    }
}
